...
impl Alpm {
    pub fn register_syncdb<S: Into<Vec<u8>>>(&self, name: S, sig_level: SigLevel) -> Result<Db> {
        ...
        let db =
            unsafe { self.lib.alpm_register_syncdb(self.as_ptr(), name.as_ptr(), sig_level.bits() as i32) };
    ...
    pub fn unregister_all_syncdbs(&mut self) -> Result<()> {
        self.check_ret(unsafe { self.lib.alpm_unregister_all_syncdbs(self.as_ptr()) })
    }
}

impl<'h> DbMut<'h> {
    pub fn unregister(self) {
        unsafe { LIBRARY.force_load().alpm_db_unregister(self.as_ptr()) };
    }

    pub fn add_server<S: Into<Vec<u8>>>(&self, server: S) -> Result<()> {
        let server = CString::new(server).unwrap();
        let ret = unsafe { LIBRARY.force_load().alpm_db_add_server(self.as_ptr(), server.as_ptr()) };
    ...
    pub fn set_servers<'a, L: IntoRawAlpmList<'a, String>>(&self, list: L) -> Result<()> {
        ...
        let ret = unsafe { LIBRARY.force_load().alpm_db_set_servers(self.as_ptr(), list.list()) };
        self.handle.check_ret(ret)
    }

    pub fn remove_server<S: Into<Vec<u8>>>(&self, server: S) -> Result<()> {
        let server = CString::new(server).unwrap();
        let ret = unsafe { LIBRARY.force_load().alpm_db_remove_server(self.as_ptr(), server.as_ptr()) };
        self.handle.check_ret(ret)
    }
}
...
impl<'h> Db<'h> {
    ...
    pub fn name(&self) -> &'h str {
        let name = unsafe { LIBRARY.force_load().alpm_db_get_name(self.as_ptr()) };
        unsafe { from_cstr(name) }
    }

    pub fn servers(&self) -> AlpmList<'h, &'h str> {
        let list = unsafe { LIBRARY.force_load().alpm_db_get_servers(self.as_ptr()) };
        unsafe { AlpmList::from_parts(self.handle, list) }
    }

    pub fn pkg<S: Into<Vec<u8>>>(&self, name: S) -> Result<Package<'h>> {
        let name = CString::new(name).unwrap();
        let pkg = unsafe { LIBRARY.force_load().alpm_db_get_pkg(self.as_ptr(), name.as_ptr()) };
    ...
    pub fn pkgs(&self) -> AlpmList<'h, Package<'h>> {
        let pkgs = unsafe { LIBRARY.force_load().alpm_db_get_pkgcache(self.as_ptr()) };
        unsafe { AlpmList::from_parts(self.handle, pkgs) }
    }

    pub fn group<S: Into<Vec<u8>>>(&self, name: S) -> Result<Group<'h>> {
        let name = CString::new(name).unwrap();
        let group = unsafe { LIBRARY.force_load().alpm_db_get_group(self.as_ptr(), name.as_ptr()) };
        self.handle.check_null(group)?;
        unsafe { Ok(Group::new(self.handle, group)) }
    }

    pub fn set_usage(&self, usage: Usage) -> Result<()> {
        let ret = unsafe { LIBRARY.force_load().alpm_db_set_usage(self.as_ptr(), usage.bits() as i32) };
        self.handle.check_ret(ret)
    }
        ...
        L: IntoRawAlpmList<'h, String>,
    {
        ...
        let list = unsafe { list.into_raw_alpm_list() };
        let ok = unsafe { LIBRARY.force_load().alpm_db_search(self.as_ptr(), list.list(), &mut ret) };
    ...
    pub fn groups(&self) -> Result<AlpmListMut<'h, Group<'h>>> {
        let groups = unsafe { LIBRARY.force_load().alpm_db_get_groupcache(self.as_ptr()) };
        self.handle.check_null(groups)?;
        unsafe { Ok(AlpmListMut::from_parts(self.handle, groups)) }
    }

    pub fn siglevel(&self) -> SigLevel {
        let siglevel = unsafe { LIBRARY.force_load().alpm_db_get_siglevel(self.as_ptr()) };
        SigLevel::from_bits(siglevel as u32).unwrap()
    }

    pub fn is_valid(&self) -> Result<()> {
        let ret = unsafe { LIBRARY.force_load().alpm_db_get_valid(self.as_ptr()) };
        self.handle.check_ret(ret)
    }
    ...
    pub fn usage(&self) -> Result<Usage> {
        ...
        let ret = unsafe { LIBRARY.force_load().alpm_db_get_usage(self.as_ptr(), &mut usage) };
        self.handle.check_ret(ret)?;
