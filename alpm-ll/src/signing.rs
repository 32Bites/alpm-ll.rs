...
use alpm_sys_ll::_alpm_sigstatus_t::*;
use alpm_sys_ll::_alpm_sigvalidity_t::*;
use alpm_sys_ll::*;
...
pub fn decode_signature<S: Into<Vec<u8>>>(
    ...
    let mut len = 0;
    let ret = unsafe { LIBRARY.force_load().alpm_decode_signature(b64.as_ptr(), &mut data, &mut len) };
    if ret != 0 {
...
impl Drop for SigList {
    fn drop(&mut self) {
        unsafe { LIBRARY.force_load().alpm_siglist_cleanup(&mut self.inner) };
    }
}
...
impl<'a> Package<'a> {
    pub fn check_signature(&self) -> Result<(bool, SigList)> {
        ...
        let ret = unsafe { LIBRARY.force_load().alpm_pkg_check_pgp_signature(self.pkg.as_ptr(), &mut siglist.inner) };
        let valid = match ret {
            0 => true,
            1 => false,
...
impl<'a> Db<'a> {
    pub fn check_signature(&self) -> Result<(bool, SigList)> {
        let mut siglist = SigList::new();
        let ret = unsafe { LIBRARY.force_load().alpm_db_check_pgp_signature(self.as_ptr(), &mut siglist.inner) };
        let valid = match ret {
            0 => true,
            1 => false,
            _ => return Err(self.handle.last_error()),
