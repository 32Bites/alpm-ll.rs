...
impl Clone for Depend {
    fn clone(&self) -> Self {
        let ptr = unsafe { LIBRARY.force_load().alpm_dep_compute_string(self.inner.as_ptr()) };
        assert!(!ptr.is_null(), "failed to compute string for dep");
        let dep = unsafe { LIBRARY.force_load().alpm_dep_from_string(ptr) };
...
impl Drop for Depend {
    fn drop(&mut self) {
        unsafe { LIBRARY.force_load().alpm_dep_free(self.dep.as_ptr()) }
    }
}
...
impl<'a> fmt::Display for Dep<'a> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        unsafe {
            let cs = LIBRARY.force_load().alpm_dep_compute_string(self.as_ptr());
...
impl<'a> From<Dep<'a>> for Vec<u8> {
    fn from(dep: Dep<'a>) -> Vec<u8> {
        unsafe {
            let cs = LIBRARY.force_load().alpm_dep_compute_string(dep.as_ptr());
            assert!(!cs.is_null(), "failed to compute string for dep");
...
impl Depend {
    pub fn new<S: Into<Vec<u8>>>(s: S) -> Depend {
        ...
        let dep = unsafe { LIBRARY.force_load().alpm_dep_from_string(s.as_ptr()) };
        assert!(!dep.is_null(), "failed to create dep from string");
        unsafe {
            Depend {
...
impl Drop for DependMissing {
    fn drop(&mut self) {
        unsafe { LIBRARY.force_load().alpm_depmissing_free(self.inner.as_ptr()) }
    }
}
...
impl<'a> AlpmList<'a, Db<'a>> {
    pub fn find_satisfier<S: Into<Vec<u8>>>(&self, dep: S) -> Option<Package<'a>> {
        ...
        let pkg = unsafe { LIBRARY.force_load().alpm_find_dbs_satisfier(self.handle.as_ptr(), self.list, dep.as_ptr()) };
...
impl<'a> AlpmList<'a, Package<'a>> {
    pub fn find_satisfier<S: Into<Vec<u8>>>(&self, dep: S) -> Option<Package<'a>> {
        ...
        let pkg = unsafe { LIBRARY.force_load().alpm_find_satisfier(self.list, dep.as_ptr()) };
        self.handle.check_null(pkg).ok()?;
        unsafe { Some(Package::new(self.handle, pkg)) }
    }
}
...
impl Alpm {
    pub fn check_deps<'a>(
        ...
        let ret = unsafe {
            LIBRARY.force_load().alpm_checkdeps(
                self.as_ptr(),
                pkgs.list(),
                rem.list(),
                upgrade.list(),
