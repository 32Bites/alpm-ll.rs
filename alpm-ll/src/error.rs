use crate::{Alpm, LIBRARY, Library};
...
impl Alpm {
    pub fn last_error(&self) -> Error {
        unsafe { Error::new(self.lib.alpm_errno(self.as_ptr())) }
    }
}
...
impl fmt::Display for Error {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        ...
        let s = unsafe { CStr::from_ptr(LIBRARY.force_load().alpm_strerror(err)) };
