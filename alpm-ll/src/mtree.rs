...
impl<'h> Drop for MTree<'h> {
    fn drop(&mut self) {
        unsafe { LIBRARY.force_load().alpm_pkg_mtree_close(self.pkg.as_ptr(), self.as_ptr()) };
    }
}
...
impl<'h> Iterator for MTree<'h> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let ret = unsafe { LIBRARY.force_load().alpm_pkg_mtree_next(self.pkg.as_ptr(), self.as_ptr(), &mut entry) };
