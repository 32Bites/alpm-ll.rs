use crate::{
    ...
    Group, LoadedPackage, OwnedConflict, OwnedFileConflict, Package, Pkg, LIBRARY, Library
};
    ...
    fn drop(&mut self) {
        if D::DROP {
            if let Some(free) = T::FREE {
                unsafe { LIBRARY.force_load().alpm_list_free_inner(self.list, Some(free)) }
            }
            unsafe { LIBRARY.force_load().alpm_list_free(self.list) };
        }
    }
}
    ...
    T::Output: AsAlpmListItemPtr<'a>,
{
    ...
    unsafe fn into_raw_alpm_list(self) -> RawAlpmList<'a, T::Output, Self::Drop> {
        ...
        for item in self {
            list = LIBRARY.force_load().alpm_list_add(list, item.as_ptr());
            if T::FREE.is_none() {
                std::mem::forget(item);
            }
        }
    ...
    for<'b> T: IntoAlpmListItem<'a, 'b>,
{
    fn drop(&mut self) {
        ...
        unsafe { LIBRARY.force_load().alpm_list_free(list) }
    }
}
    ...
    T: IntoAlpmListItem<'a, 'b>,
{
    pub fn len(&self) -> usize {
        unsafe { LIBRARY.force_load().alpm_list_count(self.list) }
    }
    ...
    pub fn last(&'b self) -> Option<T::Borrow> {
        let item = unsafe { LIBRARY.force_load().alpm_list_last(self.list) };
        if item.is_null() {
            None
        } else {
    ...
    for<'b> T: IntoAlpmListItem<'a, 'b>,
{
    ...
    pub fn to_list_mut(&self) -> AlpmListMut<'a, T> {
        let list = unsafe { LIBRARY.force_load().alpm_list_copy(self.list) };
        AlpmListMut {
    ...
    for<'b> T: IntoAlpmListItem<'a, 'b> + Push<'a> + AsAlpmListItemPtr<'a>,
{
    pub fn push(&mut self, t: T) {
        unsafe { self.list.list = LIBRARY.force_load().alpm_list_add(self.list.list, t.as_ptr()) };
        if T::FREE.is_none() {
            std::mem::forget(t);
        }
    }
...
impl<'a> AlpmListMut<'a, String> {
    pub fn push_str(&mut self, s: &str) {
        ...
        unsafe { self.list.list = LIBRARY.force_load().alpm_list_add(self.list.list, s as *mut c_void) };
    }
}
    ...
    for<'b> T: IntoAlpmListItem<'a, 'b>,
{
        ...
        F: FnMut(&T) -> bool,
    {
        ...
        while !curr.is_null() {
            ...
            if !f(&item) {
                drop(item);
                unsafe { list = LIBRARY.force_load().alpm_list_remove_item(list, curr) };
    ...
    pub fn remove(&mut self, n: usize) -> Option<T> {
        ...
        let item = unsafe { LIBRARY.force_load().alpm_list_nth(self.list.list, n) };
        unsafe { self.list.list = LIBRARY.force_load().alpm_list_remove_item(self.list.list, item) };
        let ret = unsafe {
    ...
    pub fn remove_list(&mut self, n: usize) -> AlpmListMut<'a, T> {
        ...
        let item = unsafe { LIBRARY.force_load().alpm_list_nth(self.list.list, n) };
        self.list.list = unsafe { LIBRARY.force_load().alpm_list_remove_item(self.list.list, item) };
    ...
    T: IntoAlpmListItem<'a, 'b>,
{
    fn next_data(&mut self) -> Option<*mut c_void> {
        if self.current.is_null() {
            ...
            self.current = unsafe { LIBRARY.force_load().alpm_list_next(self.current) };
    ...
    for<'b> T: IntoAlpmListItem<'a, 'b>,
{
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        let size = unsafe { LIBRARY.force_load().alpm_list_count(self.list.list.list) };
        (size, Some(size))
    }
}
    ...
    T: IntoAlpmListItem<'a, 'b>,
{
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        let size = unsafe { LIBRARY.force_load().alpm_list_count(self.list.list) };
        (size, Some(size))
    }
}
