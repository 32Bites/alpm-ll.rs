...
impl Ver {
    ...
    pub fn vercmp<V: AsRef<Ver>>(&self, other: V) -> Ordering {
        unsafe { LIBRARY.force_load().alpm_pkg_vercmp(self.0.as_ptr(), other.as_ref().0.as_ptr()).cmp(&0) }
    }
...
impl PartialOrd for Ver {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        unsafe { LIBRARY.force_load().alpm_pkg_vercmp(self.0.as_ptr(), other.0.as_ptr()).partial_cmp(&0) }
    }
}
...
impl PartialEq for Ver {
    fn eq(&self, other: &Self) -> bool {
        unsafe { LIBRARY.force_load().alpm_pkg_vercmp(self.0.as_ptr(), other.0.as_ptr()) == 0 }
    }
}
