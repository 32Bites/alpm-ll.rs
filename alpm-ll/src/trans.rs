...
use alpm_sys_ll::_alpm_transflag_t::*;
use alpm_sys_ll::*;
...
impl Alpm {
    pub fn trans_flags(self) -> TransFlag {
        let flags = unsafe { self.lib.alpm_trans_get_flags(self.as_ptr()) };
        TransFlag::from_bits(flags as u32).unwrap()
    }

    pub fn trans_prepare(&mut self) -> std::result::Result<(), (PrepareResult, Error)> {
        let mut list = ptr::null_mut();
        let ret = unsafe { self.lib.alpm_trans_prepare(self.as_ptr(), &mut list) };
    ...
    pub fn trans_commit(&mut self) -> std::result::Result<(), (CommitResult, Error)> {
        let mut list = ptr::null_mut();
        let ret = unsafe { self.lib.alpm_trans_commit(self.as_ptr(), &mut list) };
        let err = self.check_ret(ret);
    ...
    pub fn trans_interrupt(&mut self) -> Result<()> {
        let ret = unsafe { self.lib.alpm_trans_interrupt(self.as_ptr()) };
        self.check_ret(ret)
    }

    pub fn trans_add(&self) -> AlpmList<Package> {
        let list = unsafe { self.lib.alpm_trans_get_add(self.as_ptr()) };
        unsafe { AlpmList::from_parts(self, list) }
    }

    pub fn trans_remove(&self) -> AlpmList<Package> {
        let list = unsafe { self.lib.alpm_trans_get_remove(self.as_ptr()) };
        unsafe { AlpmList::from_parts(self, list) }
    }

    pub fn trans_release(&mut self) -> Result<()> {
        let ret = unsafe { self.lib.alpm_trans_release(self.as_ptr()) };
        self.check_ret(ret)
    }
}
...
impl Alpm {
    pub fn trans_init(&self, flags: TransFlag) -> Result<()> {
        let ret = unsafe { self.lib.alpm_trans_init(self.as_ptr(), flags.bits() as i32) };
        self.check_ret(ret)
    }
}
